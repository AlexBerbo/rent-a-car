...
<?php
	...
	if(isset($_POST['login'])){ // when click on Update button
		...
		$result= "SELECT * FROM  admin WHERE username = '$korisnickoime'";
		...
		if($newPw==$cPw){
			if($username==$user){
				$qry = mysqli_query($konekcija, "select * from admin where username='$user'"); // select query
			$data=$qry->fetch_assoc();
   	 		$upit="UPDATE admin SET password='$newPw' WHERE username='$user'";
