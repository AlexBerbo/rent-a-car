...
<?php
	...
	if(isset($_POST['login'])){
		...
		if($newPw==$cPw){
			if($username==$user){
				$qry = mysqli_query($konekcija, "select * from korisnik where username='$user'"); // select query
			$data=$qry->fetch_assoc();
   	 		$upit="UPDATE korisnik SET password='$newPw' WHERE username='$user'";
...
<header class="header">
	<div class="slika">
		...
		<ul>
			...
			<li>
				<a class="nav-bar" href="register.php">Registruj se</a>
			</li>
		</ul>
		</nav>
		<div class="zavesa">
...
<section id="login" class="login-section">
	...
	<div class="login-div">
		...
		<h2>PROMENI LOZINKU</h2>
			...
			<p>Potvrdi lozinku: <input type="password" name="cpassword" /></p>
